...
describe("extension-dialog-card select dialog", () => {
  ...
  it("renders mobile-safe permission labels but submits opaque choice ids without a cancel control", async () => {
    ...
    const actions = requiredElement(root.querySelector(".dialog-action-stack"), "permission actions");
    expect([...actions.querySelectorAll("button")].map((button) => button.textContent.trim())).toEqual(["Yes", "No"]);
